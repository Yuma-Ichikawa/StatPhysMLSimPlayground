...
\paragraph{1. A susceptibility peak from a handful of seeds is often one run.}
Across the release, \AtlasSpikeRejected{} of \AtlasSpikeTotal{} size-curves have
their global susceptibility maximum at an isolated spike: one diverged run
inflates the variance at exactly one control value, and the neighbours sit at
the floor. At eight outer seeds the rate was one in four; going to twelve
lowered it, which is the cleanest evidence we have that these are replication
artefacts rather than physics. A plain argmax reports each of these as a boundary, and its
