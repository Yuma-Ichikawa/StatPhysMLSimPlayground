...
$\AtlasGrokkingSemanticRatio{}$ times its interval. The susceptibility of the
delay peaks in the interior at two of the three training-set sizes; at the
third the maximum is an isolated spike and is rejected, so no sharpening
exponent is fitted. Verdict \emph{\AtlasGrokkingVerdict{}}.

\paragraph{Interpretation.}
Delayed generalisation exists and is controlled: the delay varies far outside
seed noise across the weight-decay axis, and the run-to-run spread peaks in the
interior rather than at an endpoint. Whether that spread \emph{diverges} with
size is not established at this replication.
...
risk varies over $\AtlasSgdTemperatureSemanticRatio{}$ times its interval, but
its susceptibility maxima fall at the edge of the sampled temperature range and
are censored. Verdict \emph{\AtlasSgdTemperatureVerdict{}}, grade
\AtlasSgdTemperatureGrade{}.

\paragraph{Interpretation.}
The stationary state responds strongly to the effective temperature, and the
...
\paragraph{Not established.}
A critical temperature. The response is unambiguous but its maxima sit outside
the sampled window, so nothing is located. At eight seeds this family did show
an interior peak that sharpened with size; at twelve it does not, which is the
right direction for a diagnostic to move when the extra seeds reveal that the
earlier maximum was noise. We also do not claim the $(\eta,B)$ collapse is
exact, only that the design can test it.
