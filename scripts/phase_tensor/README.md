...
through `submit.sh` with a deployment-owned `ARRAY_SCRIPT`; the adapter defines
the Slurm partition and hardware policy. After all registered-seed tasks finish,
