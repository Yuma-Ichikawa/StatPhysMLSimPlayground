...
cd "$REPO_ROOT"
families=(tensor_confirm_mlp tensor_confirm_optimizer tensor_confirm_scaling tensor_confirm_natural)
for family in "${families[@]}"; do
  "$PYTHON" -m statphys.continuation.cli expand \
    "experiments/phase_continuation/configs/confirmation/${family}.toml" \
...
"$PYTHON" -m statphys.continuation.cli compose "${inputs[@]}" \
  --study empirical_phase_confirmation_v3 \
