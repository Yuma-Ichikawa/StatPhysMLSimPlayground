...
from statphys.simulation import OnlineSimulation, ReplicaSimulation, SimulationConfig
...
def test_replica_simulation():
    ...
    for i, (ax, name, color) in enumerate(zip(axes, param_names, colors, strict=False)):
        ax.errorbar(
            alpha_values,
            op_mean[:, i],
            yerr=op_std[:, i],
...
def test_models():
    ...
    for i, (name, p) in enumerate(zip(names, params, strict=False)):
