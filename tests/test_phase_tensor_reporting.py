...
def test_causal_metrics_store_raw_risks_deltas_and_bounded_effects() -> None:
    ...
    assert all(-1.0 <= value <= 1.0 for name, value in metrics.items() if name.endswith("_effect"))
        ...
        for rate in (0.01, 0.1):
            ...
            candidates.append(
                _condition(
                    ...
                    control,
                    {
                        "normalized_test_risk": _estimate(
                            calibration_risk if size == 64 else endpoint_risk
                        )
                    },
                    size=size,
                    learning_rate=rate,
...
def test_plot_contract_bounded_censoring_scaling_and_paired_coverage(
        ...
        "thresholds": [
            {
                "family": "tensor_mlp",
                "size": 64,
                "parameters": {"data_kind": "synthetic_retrieval"},
    ...
    assert scaling_labels == {
        r"model width $d_{\rm model}$",
        r"depth $L$",
        r"context length $T_{\rm ctx}$",
        r"training examples $N_{\rm train}$",
    }
