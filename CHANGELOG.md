...
### Added
- **`statphys.continuation`, `statphys.phase_tensor`, `statphys.predictive`,
  `statphys.atlas`: the phase-continuation research program** — four new
  subpackages and two new console scripts (`phase-continuation`,
  `phase-tensor`) implementing a unified numerical program for realism-tier
  studies beyond the frontier paradigms above, documented in
  `docs/phase_continuation.md`:
  - `statphys.continuation` (`core/`, `domains/`, `analysis/`,
    `orchestration/`): immutable task schemas, numerical domains
    (transformer / diffusion / reinforcement / multiagent / cross-domain,
    each spanning exact/oracle, reduced-order, and learned-neural-bridge
    tiers), strict aggregation, coverage and taxonomy auditing, and
    Slurm/Spark-portable execution with no hardcoded cluster paths. Every
    confirmatory condition uses exactly five distinct, SHA-256-derived
    outer seeds; `phase-continuation coverage` and `phase-continuation
    taxonomy` audit manifests before submission
  - `statphys.phase_tensor` (`phase-tensor` CLI): a portable finite-width
    tensor reference study (data/model/observables/optimizers/runner/
    report/paper), reproducible end to end via `scripts/run_gpu_reference.sh`
    or the Slurm/Spark scripts in `scripts/phase_tensor/`; its 45-task,
    five-outer-seed validation run and self-contained HTML report are
    published under `evidence/tensor_reference_validation/`
  - `statphys.predictive`: a predictive phase-continuation pipeline
    (`pipeline.py`, `simulators.py`, `schema.py`) with its own confirmation
    suite and manuscript contribution
  - `statphys.atlas`: an audited scientific atlas of attention-ladder
    sweeps (`models/`, `data/`, `observables/`, `analysis/`) with GPU
    evidence, Slurm/cluster orchestration (`cluster.py`), and strict
    aggregation/plotting
  - `statphys.core`: program-agnostic evidence, registry, and provenance
    contracts (`contracts.py`, `evidence.py`, `registry.py`, `rng.py`, ...)
    shared by `continuation`, `phase_tensor`, `predictive`, and `atlas`
  - Guided-workflow CLI hardening: `preview`, `retry`, `status`,
    `aggregate`, `paper`, and `lab` subcommands added to `statphys`
    alongside `catalog`/`new`/`validate`/`run`/`inspect`/`report`/`resume`/
    `compare`/`doctor` (see `docs/guided_workflow.md`)
  - The manuscript (`paper/`) was retitled and substantially expanded to
    *An Evidence-Aware Statistical-Mechanics Atlas for Modern Machine
    Learning*, covering the phase-continuation program, the audited atlas,
    and their five-seed confirmatory results, with figures and TeX macros
    regenerated from the registered strict aggregates
    (`scripts/generate_paper_figures.py`)
- **`statphys.frontier`: modern learning paradigms as teacher-student
  physics** — a new subpackage that measures theoretically intractable
  training paradigms with the same order parameters (function-space
  overlaps, generalization error, phase diagrams) used in the exactly
  solvable settings. Five settings, each with `run_*`/`sweep_*` APIs, a
  CLI study, a figure, and full math in the new `docs/frontier.md`:
  - `frontier.sft` (`statphys study sft`): fine-tuning as a two-teacher
    problem with an exact task-similarity dial (`correlated_teacher`);
    catastrophic-forgetting phase diagram and the positive/negative
    transfer sign boundary in the (similarity, alpha_ft) plane
  - `frontier.rlhf` (`statphys study rlhf`): reward-model
    overoptimization — a hackable gold reward (teacher + off-
    distribution penalty), Bradley-Terry preference training, and two
    policies with exact KL budgets: a KL-regularized Gaussian shift
    policy (can leave the data distribution; shows the full Goodhart
    turnover) and best-of-n (stays on-support; no turnover at equal
    KL). The turnover point KL*(alpha_r) and peak gold reward
    G_max(alpha_r) trace the aligned/hacked phase boundary
  - `frontier.weak_to_strong` (`statphys study weak_to_strong`):
    truth -> weak supervisor -> strong student chains; performance-gap-
    recovered (PGR) surface locating the imitation-to-generalization
    crossover
  - `frontier.collapse` (`statphys study collapse`): model collapse
    under recursive synthetic-data training; overlap decay, output-
    variance (tail) collapse, and the real-data anchoring boundary
  - `frontier.icl` (`statphys study icl`): emergence of in-context
    learning in a small causal transformer vs pretraining task
    diversity, with a Bayes-optimal in-context ridge baseline and a
    ridge-alignment order parameter identifying the algorithmic
    transition from task retrieval to in-context regression
  - **teacher taxonomy** (`frontier.teachers`): a table-managed
    registry of teacher constructors — random (Gaussian MLP, linear
    index), structured (sparse / low-rank / heavy-tailed / spiked /
    binary weights), and *real-data* teachers (a network genuinely
    trained on sklearn digit images, evaluated on the real image
    manifold, plus its random-weight control). Each entry returns
    `(teacher, input_sampler, d)` and injects into every paradigm via
    the new `teacher=` / `input_sampler=` arguments
  - **taxonomy cross experiment** (`frontier.taxonomy`,
    `statphys study taxonomy`): every taxonomy teacher through every
    paradigm probe (SFT forgetting, RLHF KL*, weak-to-strong PGR,
    collapse drop), rendered as a bar-panel figure and a markdown
    table with per-cell mean ± std — the universality question
    ("does teacher structure move the frontier boundaries?") as one
    command
  - shared infrastructure in `frontier.common` (one training loop, one
    overlap measure, one `InputSampler` abstraction for data
    distributions, correlated-teacher construction) so new paradigms
    are one small module each; `FRONTIER_STUDIES` merges into the main
    `STUDIES` registry so the CLI exposes everything uniformly
  - new optional dependency extra `realdata` (scikit-learn) for the
    digit-image teachers; included in `dev`
- **Paper draft** (`paper/`): an initial manuscript on the frontier studies
  above, LaTeX source, bibliography, and figures all generated by
  `statphys study ...`. The manuscript was later retitled and expanded (see
  the phase-continuation entry above) to *An Evidence-Aware
  Statistical-Mechanics Atlas for Modern Machine Learning*
  - README frontier gallery (rlhf / collapse / icl figures) and
    `tests/test_frontier.py` (16 tests)
...
[Unreleased]: https://github.com/Yuma-Ichikawa/StatPhysMLSimPlayground/compare/v0.1.0...HEAD
[0.1.0]: https://github.com/Yuma-Ichikawa/StatPhysMLSimPlayground/releases/tag/v0.1.0
