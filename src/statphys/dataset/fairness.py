...
class TeacherMixtureFairnessDataset(BaseDataset):
    ...
    def compute_fairness_metrics(self, predictions: torch.Tensor, labels: torch.Tensor, 
                                  groups: torch.Tensor) -> Dict[str, float]:
        ...
        pred_rate_plus = (predictions[groups == 1] == 1).float().mean().item() if (groups == 1).any() else 0
        pred_rate_minus = (predictions[groups == -1] == 1).float().mean().item() if (groups == -1).any() else 0
        metrics['demographic_parity_gap'] = abs(pred_rate_plus - pred_rate_minus)
        
        return metrics
    
    def __repr__(self) -> str:
        return (f"TeacherMixtureFairnessDataset(d={self.d}, group_ratio={self.group_ratio}, "
