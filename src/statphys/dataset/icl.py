...
class ICLLinearRegressionDataset(BaseDataset):
    ...
    def generate_sample(self) -> tuple[torch.Tensor, ...]:
        ...
        if self.noise_std > 0:
            context_y = context_y + self.noise_std * torch.randn(
                self.context_len, device=self.device
            )
    ...
    def generate_dataset(self, n_samples: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        X = torch.cat([batch["context_x"].reshape(n_samples, -1), batch["query_x"]], dim=1)
        y = batch["query_y"]
        return X, y
    ...
    def __repr__(self) -> str:
        return (
            f"ICLLinearRegressionDataset(d={self.d}, context_len={self.context_len}, "
...
class ICLNonlinearRegressionDataset(BaseDataset):
    ...
    def __repr__(self) -> str:
        return (
            f"ICLNonlinearRegressionDataset(d={self.d}, k={self.k}, "
