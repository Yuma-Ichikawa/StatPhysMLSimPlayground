...
class MarkovChainDataset(BaseDataset):
    ...
    def __init__(
        ...
        self.state_embeddings = self.state_embeddings / self.state_embeddings.norm(dim=1, keepdim=True)
    ...
    def __repr__(self) -> str:
        return (f"MarkovChainDataset(d={self.d}, n_states={self.n_states}, "
...
class CopyTaskDataset(BaseDataset):
    ...
    def generate_sample(self) -> Tuple[torch.Tensor, ...]:
        ...
        return (batch['tokens'][0], batch['token_ids'][0], 
                batch['triggers'][0], batch['targets'][0])
    
    def generate_batch(self, batch_size: int) -> Dict[str, torch.Tensor]:
        """Generate copy task sequences."""
        tokens = torch.randint(0, self.vocab_size, (batch_size, self.seq_len), device=self.device)
        triggers = torch.randint(0, self.vocab_size, (batch_size, self.n_triggers), device=self.device)
        targets = torch.randint(0, self.vocab_size, (batch_size, self.n_triggers), device=self.device)
        
        first_positions = torch.randint(0, self.seq_len // 2 - 1, (batch_size, self.n_triggers), device=self.device)
        second_positions = torch.randint(self.seq_len // 2, self.seq_len - 1, (batch_size, self.n_triggers), device=self.device)
    ...
    def __repr__(self) -> str:
        return (f"CopyTaskDataset(d={self.d}, vocab_size={self.vocab_size}, "
...
class GeneralizedPottsDataset(BaseDataset):
    ...
    def __repr__(self) -> str:
        return (f"GeneralizedPottsDataset(d={self.d}, seq_len={self.seq_len}, "
...
class TiedLowRankAttentionDataset(BaseDataset):
    ...
    def __init__(
        ...
        seq_len: int = 10,
        teacher_rank: int = 5,
        position_covariances: Optional[List[torch.Tensor]] = None,
    ...
    def __repr__(self) -> str:
        return (f"TiedLowRankAttentionDataset(d={self.d}, seq_len={self.seq_len}, "
...
class MixedGaussianSequenceDataset(BaseDataset):
    ...
    def __repr__(self) -> str:
        return (f"MixedGaussianSequenceDataset(d={self.d}, seq_len={self.seq_len}, "
