...
def parser() -> argparse.ArgumentParser:
    ...
    adaptive = commands.add_parser("adaptive-manifest")
    adaptive.add_argument("--base-manifest", required=True)
    adaptive.add_argument("--aggregate", required=True)
    ...
    aggregate_command.add_argument("--manifest", required=True)
    aggregate_command.add_argument("--runs", required=True)
    aggregate_command.add_argument("--output", required=True)
    audit = commands.add_parser("audit")
    audit.add_argument("--aggregate", required=True)
    ...
    paper.add_argument("--aggregate", required=True)
    paper.add_argument("--audit", required=True)
    paper.add_argument("--output", required=True)
    slurm = commands.add_parser("slurm")
    slurm.add_argument("--manifest", required=True)
    slurm.add_argument("--profile", required=True)
...
def main() -> None:
    ...
    if args.command == "manifest":
        result = {"manifest": str(build_manifest(args.config).write(args.output))}
        ...
        result = run_slice(args.manifest, args.output, args.start, args.stop, args.device)
    elif args.command == "aggregate":
        payload = aggregate(args.manifest, args.runs, args.output)
        result = {
            "output": args.output,
            "tasks": payload["registered_tasks"],
        ...
        result = {
            "output": args.output,
            "metric_groups": len(payload["statistics"]),
            "saturated_groups": len(payload["saturated_metrics"]),
        }
    elif args.command == "plot":
        result = {"figures": [str(path) for path in plot_results(args.aggregate, args.output)]}
    elif args.command == "paper":
        result = {"results": str(write_paper_results(args.aggregate, args.audit, args.output))}
    else:
        result = {"script": str(render_slurm(args.manifest, args.profile, args.output))}
