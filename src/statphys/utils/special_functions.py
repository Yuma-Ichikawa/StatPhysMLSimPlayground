...
def gaussian_pdf(x: Union[float, np.ndarray], mu: float = 0.0, sigma: float = 1.0) -> Union[float, np.ndarray]:
    """
...
def gaussian_cdf(x: Union[float, np.ndarray], mu: float = 0.0, sigma: float = 1.0) -> Union[float, np.ndarray]:
    """
...
def gaussian_tail(x: Union[float, np.ndarray], mu: float = 0.0, sigma: float = 1.0) -> Union[float, np.ndarray]:
    """
...
def gaussian_quantile(p: Union[float, np.ndarray], mu: float = 0.0, sigma: float = 1.0) -> Union[float, np.ndarray]:
    """
...
def erf_activation(x: Union[float, np.ndarray]) -> Union[float, np.ndarray]:
    """
...
def erf_derivative(x: Union[float, np.ndarray]) -> Union[float, np.ndarray]:
    """
...
def softplus(x: Union[float, np.ndarray]) -> Union[float, np.ndarray]:
    """
...
def soft_threshold(x: Union[float, np.ndarray], threshold: float) -> Union[float, np.ndarray]:
    """
...
def firm_threshold(x: Union[float, np.ndarray], lambda1: float, lambda2: float) -> Union[float, np.ndarray]:
    """
    ...
    result = np.where((abs_x > lambda1) & (abs_x <= lambda2),
                      np.sign(x) * (abs_x - lambda1) * lambda2 / (lambda2 - lambda1),
                      result)
    return result


def moreau_envelope(func: Callable[[float], float], x: float, gamma: float, n_points: int = 100) -> Tuple[float, float]:
    """
...
def training_error_linear(m: float, q: float, rho: float, eta: float, alpha: float, reg_param: float = 0.0) -> float:
    """
    ...
    if alpha > 0:
        e_train = V * reg_param**2 / (alpha * (reg_param + alpha)**2 + 1e-10)
    else:
        e_train = 0.0
